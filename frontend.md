...
:root {
  ...
  --border-default: rgba(48, 54, 61, 1.0);
  ...
  --gradient-surface: linear-gradient(180deg, rgba(22,27,34,0) 0%, rgba(22,27,34,0.8) 100%);
...
:root {
  ...
  --shadow-glow: 0 0 20px rgba(88,166,255,0.15), 0 0 40px rgba(88,166,255,0.05);
...
```css
:root {
  --ease-spring:  cubic-bezier(0.34, 1.56, 0.64, 1);   /* bouncy — status changes */
...
@keyframes status-pulse {
  0%, 100% { opacity: 1; box-shadow: 0 0 0 0 var(--status-running-glow); }
...
.page-enter { animation: page-enter var(--duration-slower) var(--ease-smooth) both; }
```

**LogLineAppear** — new log lines streaming into the log viewer
```css
@keyframes log-appear {
  from { opacity: 0; transform: translateX(-4px); background: rgba(88,166,255,0.06); }
  to   { opacity: 1; transform: translateX(0);    background: transparent; }
}
.log-line-new { animation: log-appear var(--duration-slow) var(--ease-smooth) both; }
```
...
@keyframes glow-pulse {
  ...
  50%       { box-shadow: 0 0 40px rgba(88,166,255,0.25), 0 0 80px rgba(88,166,255,0.1); }
}
```
...
*Infinity Node Frontend PRD v1.0 · UI Architecture for Dashboard, Logs, Observability, and Function Upload*